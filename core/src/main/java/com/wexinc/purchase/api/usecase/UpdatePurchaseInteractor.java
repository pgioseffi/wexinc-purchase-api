...
public class UpdatePurchaseInteractor implements UpdatePurchaseInputBoundary {
	...
	/**
	 * Field that will perform a mapping action from a {@link PurchaseDTO} without a positive numeric identifier to one
	 * with.
	 *
	 * @since 1.0.0
	 ...
	 * @param purchaseCoreMapperParam Parameter responsible for holding the mapper that will initialize the
	 *                                {@link #purchaseCoreMapper} field of this class.
	 * @since 1.0.0
	 */
	public UpdatePurchaseInteractor(final PurchaseGateway purchaseGatewayParam,
	...
	public PurchaseDTO apply(final Long id, final PurchaseDTO purchaseDTO) {
		...
		throw new EntityNotFoundException(ConstantsCore.EXCEPTION_MESSAGE_PURCHASE_NOT_FOUND.formatted(id));
	}
}
